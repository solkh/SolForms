...
{    
    ...
    {        
        ...
        #region Submittions
        public async Task<SFSubmition?> GetSubmittion(Guid sessionId) => 
            await _dataSource.Get<SFSubmition>(sessionId);
        public async Task<SFSubmition?[]> GetSubmittions(Guid formId) =>
            await _dataSource.GetAll<SFSubmition>(formId) ?? Array.Empty<SFSubmition>();
        public async Task<int> CountSubmittions(Guid formID) =>
            await _dataSource.Count<SFSubmition>();
        public async Task SubmitForm(SFSubmition answeringSession) => 
            await _dataSource.Create(answeringSession);
        public async Task UpdateSubmission(SFSubmition answeringSession) => 
            await _dataSource.Update(answeringSession.Id ?? Guid.Empty, answeringSession);
        public async Task UpdateSubmission(Guid id, SFSubmition answeringSession) => 
            await _dataSource.Update(id, answeringSession);
        public async Task<bool> DeleteSubmittion(Guid sessionId) => 
            await _dataSource.Delete<SFSubmition>(sessionId);
        #endregion
